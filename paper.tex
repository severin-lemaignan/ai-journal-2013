...
\begin{tikzpicture}[
  ...
  %%% ORO
  \node at (0,0)[skill, ultra thick, fill=LimeGreen!50] (oro) {Symbolic facts \\ and beliefs management};
  
  %%% HATP
  \node at (-6, 2)[skill, fill=PineGreen!50] (hatp) {Human-aware symbolic \\ task planning};
  
  %%% DIALOGS
  \node at (-6, -2) [skill, fill=BrickRed!50] (dialogs) {Dialogue processing};
...
\begin{tikzpicture}[
    >=latex,
    every edge/.style={<-, draw, very thick},
    every node/.style={draw, font=\tt, node distance=0.5, rounded corners,
