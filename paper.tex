...
plausible model of human cognition.  The main design principles where
to nurture the decisional components of the robot with models of human
behaviour and human preferences in order to develop a pertinent
cognition for a robot that is able to serve and interact fluently with
humans. In this sense, it aims at the same objectives as\cite{Fong2005,trafton2013act}.

This architecture is based on the same principles as a number of known robotic
layered architectures~\cite{Alami98,Gat1998three,Volpe2001CLARAty, Goldberg2002}.

However, the main point for us was to refine and study in detail the
``internal'' architecture of the deliberative level.
The interactions between components at the deliberative level
are essentially bidirectional and we do not introduce layers of abstraction
...
Another example is the intricate relation between high-level symbolic
planning and geometric planning to deal with affordances and human preferences.
