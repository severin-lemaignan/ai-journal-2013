...
Human-Robot interaction is an area full of challenges for artificial
...
several mental models, pertinent situation assessment skills...the list goes on.
...
grounded amodal representations~\cite{Mavridis2006}, semantic
...
placements can provide clues for high level human monitoring
...
a \emph{throw}). {\sc Spark} recognises a handful \fxfatal{which ones?} of these
...
planner (for Human Aware Task Planner~\cite{Alili2008,
Alili2009,Lallement2014}).  

HATP (Hierarchical Agent- based Task Planner) planning framework extends the
...
parallel and synchronised when necessary.
...
top, robot actions at the bottom). The arrow between the two streams represents
a synchronization point ({\sf PUTRV} stands for {\it Put it so it is both
...
To address some of those challenges HATP includes mechanisms called
"social tules" to filter plans so as to keep only those suitable for
HRI. To this end, HATP allows the specification of the following
filtering criteria: 

\emph{Wasted time}: Avoids plans where the human mentioned in a plan spends a lot
of its time waiting.  

\emph{Effort balancing:} Avoids plans where efforts are
not fairly distributed among the agents mentioned in a plan.  It is
...
Combining some of the above criteria could help yield the
following interesting \emph{shared plans}: (1) the human ends up doing a
number of the tasks, but yet the overall effort is significantly lower than what the robot puts
to do a lower number of effort-intensive tasks; (2) avoiding, when
possible, having the human wait for the robot several times, which
essentially prevents the streams from having too many causal links
between them.

 In the current implementation, the so-called social rules are
implemented by looking through all the plans produced and filtering
out the ones that do not meet the requirements specified. In the
...
one (Figure~\ref{plan_hatp1}) where the \emph{no wasted time} social rule is
applied. The resulting shared plan is a better plan according to a global evaluation of
the costs and rules.
...
the plans are directly communicated to the robot controller. This leads to
independent declarative and procedural knowledge stores.
...
suited for prototyping.  They both rely on deep integration with the knowledge
base that serves as the primary source of semantics for decision making.
...
contex-based refinement and robot actions execution as well as monitoring of
...
parametrization allowing to adapt to various environments, and various levels of
involvement of the robot ranging from teammate behavior to assistant or
proactive helper.  Depending on this, the robot controller invokes the various
human-aware planners and will react to events detected at ORO level.

SHARY’S originality, as a supervision system, lies in its ability to take into
account not only the task achievement but also communication and monitoring
needed to support interactive task achievement ~\cite{Rich1997,Sidner2005} in a
flexible way. SHARY allows to link action execution to more or less elaborated
“communication policies” in order to produce signals towards the human and to
react to human action.

A communication act is the central notion in the formalism. It plays the role of
transition condition. Consequently, a communication act represents an
information exchange between the two agents. This exchange can be realized
through dialog or by an expressive motion or a combination of the two. It
enables each agent to communicate his belief about the task to be realized in
order to share mutual knowledge and to synchronyze their activities.  This is
done through real-time task-based situation assessment achieved by the
combination of SPARK monitoring processes and ORO inference mechanisms
\cite{fiore2014}.  
...
We first restate the challenge of \emph{embodied cognition} in general and list
what we see as the main decisional issues that need to be tackled to push
forward human-robot interaction. We then reflect on the importance of knowledge
handling in robotic architectures that deal with complex semantics and put into
perspective our choice of the RDF formalism.  Finally, we briefly re-discuss two
facets of HRI: the importance of \emph{modelling the humans}, and the difficulty
of \emph{acting in presence of humans}.
...
assessment module, {\sc Spark}.  Not only it builds symbolic facts from spatial
reasoning, but it also relies on (limited) temporal reasoning to track the world
state and build explanations to interpret unexpected perceptions (like a
suddenly disappearing object).
...
grounded: we have presented how pointing, for instance, is used by the robot to
ground \emph{``this''} or \emph{``that''} to the pointed artifact.
...
(for instance with RFID tags) is a realistic path.\footnote{It may be the case if the
...
We see however that we have only scratched the surface of a generic grounding
capability. Context, cultural background, ``naive physics'' knowledge, emotional
...
Human-robot interaction and more particularly one of it major subfields
which is human-robot collaboration provides a rich and renewed context
and challenge for the decisional abilities of a robot.
Indeed, the context where a robot has to achieve a task together with
humans gives rise to the following questions, each of them being a
challenge of is own for the robot decisional capabilities  what, who,
when; where and how ?


\emph{What to do next,} at different levels of abstractions and taking into
account several not only the current state but also the a long term
goal, is the basic or standard question for an intelligent robot. Its
is enriched here by complementary considerations dealing with the
physical and mental state of its human partner, the set of possible
actions are enriched by communication robot acts.

\emph{Who should act now} is of key importance and need also to be
decided upon by the robot. Indeed, it is sometimes better - one could
say ``more intelligent'' - for the robot to wait and leave ist human
partner act. Turn-taking opens various decisional challenges.

\emph{When to perform a given action} is enriched here by the
human-context since it has take into account the human, his needs, his
rhythms and his mental state. While performing its share of the task
the robot will have to produce signals towards the human and to
respond to signals produced by him at the proper pace.

\emph{Where to perform an action} plays an important role in presence of
humans since here again, the choice is not trivial and might need
elaborated decision. Robot here has to take into account effort
sharing, visibility of its action by the human, disturbance or
discomfort  induced by its action.

\emph{How to perform an action} plays a key role when it comes to a
robot which has to interact with humans. Very often, there are
numerous ways to perform an action or to achieve a goal. Deciding which
one to perform is a non trivial decision problem. One way to take into
account this aspect is to develop cost-based planners which tend to
search for plans that satisfy an acceptable "cost" in terms, for
instance, of ``acceptability'' or ``legibility''.

It is also clear that these different questions are not independent
and should often be considered in one decision step. The human-aware task and
motion planners, which we have built, are instances of systems which
have been specially designed to deal with this intricacy.

In one word, the quality of the decision will be measured here in
terms of ``pertinence'' of robot behaviour. This notion covefrs
several aspects as dicussed above, including efficiency. 
...
One essential and incredibly complex question is the consideration of
humans in the system, how they are modeled and how these models are
used by the robot to perceive and interpret their motion and action,
to synthesize its behaviour in order to act on the environment and to
interact with its human partners.

We claim that it is possible and useful to devise and use models of
the humans, how they act, their needs and preferences. This can be
done in very limited, but still interesting and useful, contexts such
as collaborative human-robot objects manipulation, fetch-and-carry and
associated activities in a domestic or professional environment. 

\subsubsection{Estimate human mental state}
...
still simple and mostly focused on geometric features and affordances (\emph{Who sees what? What are
our relative positions? reachability estimation}  etc.). Extending this knowledge with more subtle
perceptions (emotional state for instance) remains to be done.

\subsubsection{Motion and action execution in close vicinity to the human}

The human model we use embeds also human preferences and associated
constraints that could be taken into account when synthesizing robot motion
or elaborating robot plans.

This includes proxemics (human-robot distance) and associated issues
(visibility) but also legibility and acceptability criterias
expressed in terms of \emph{social rules} that the elaborated plans should
satisfy.


\subsubsection{Acting in Presence of Humans}

HRI offers new challenges to planning by adding a supplementary constraint: the
synthesis of plans that are not only feasible but legible (in terms of their
intention) and compliant with human preferences. This constraint is often
ill-defined, dynamic and context-dependent.  HATP, by co-planning for the robot and
the human, and modelling social rules as planning parameters, is a first step in
this direction.

\fxfatal{What are the next steps?}
