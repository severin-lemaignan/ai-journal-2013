...
and they can be analysed in terms of the cognitive skills that they require.
...
architecture, and try to highlight their significance for artificial
...
bidirectional approach.  The different components that we have mentioned so far
...
\begin{tikzpicture}[
    ...
    \node[draw=none] at (0,-1) {\includegraphics[width=0.8\columnwidth]{spark-oro-notext.pdf}};
...
updated at run-time by the robot based on its sensors (in most of
our studies, objects are identified and localised through 2D fiducial markers,
while humans are tracked with Kinect-like devices, optionally assisted by motion
...
our experiments, it runs at about 10Hz). At each steps, it re-computes spatial
...
been most beneficial.  In~\cite{Lemaignan2011a} we detail the techniques and the
...
planner (for Human Aware Task Planner~\cite{Alili2008, Alili2009}).  HATP is a
refinement of a Hierarchical Task Network (HTN). As an HTN, it performs an
...
of the central knowledge base, using a specific formalism (see the related
...
call \emph{shared plans}. This effectively enriches the interaction capabilities
...
In addition to these costs, HATP also takes into account a set of social rules.
Social rules are heuristics aiming at leading the plan construction towards the
best plan according to some human preferences.  The social rules we have defined
so far deal with \emph{undesirable states} to avoid a state in which the human
could feel uncomfortable; \emph{undesirable sequences} to eliminate sequences of
actions that can be misinterpreted by the human; \emph{effort balancing} to
adjust the work effort of each of the agents; \emph{no wasted time} to avoid
long delays between the actions of the human partner; and \emph{no intricate
links} to limit dependencies between the actions of two or more agents.
...
rely ona dedicated 3D motion planner ({\sc Mhp}, depicted in
...
\cite{Sisbot2008, Mainprice2011, Pandey2011} detail these techniques.
...
run without any geometric perception, but its disambiguation routines
