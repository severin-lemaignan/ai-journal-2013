...
Contrary to similar projects like KnowRob~\cite{Tenorth2009a} that relies on
...
of those that tries to reproduce a human-like memory organisation. The GLAIR
...
this feature during dialogue disambiguation for instance. On the other hand,
...
derived symbolic knowledge is therefore transient: it lasts only as long as the
environment remains in the same state.
...
{\sc Spark} runs and continuously updates the knowledge base at about 10Hz. At each step, it re-computes spatial
relations and affordances for the whole scene, and send the delta (new
...
performance issues in our constrained scenarios), but simplifies the
...
These type of descriptors have been studied in the context of language grounding
...
distances of the objects are taken into account.  {\sc Spark} also compute
symbolic facts related to agent independent world dynamics.  The predicate
\concept{isMoving} states, for each tracked entity, whether it is currently
moving or not.
...
such primitives that are relied upon by the execution controller to track the
state transitions in the human plans.
...
as input (processing of the human speech) and as output (verbalization of the
robot intents). Natural language processing is facilitated by our architecture
mostly processing informations with semantics that are already close to the
...
Android-based interface that relies on the Google speech recognition API for
speech-to-text and relays the textual transcript to the robot. It is parsed into
a grammatical structure (\emph{Part of Speech} tagging), and the atoms of each
sentence are resolved with the help
of the ontology to ground concepts like objects (\ie when a user says ``pick the
can'', it resolves to which instance of \emph{can} the user is referring to) and
actions.  Sentences are classified into questions, desires and statements, and
processed accordingly.  Figure~\ref{dialogs|ex} gives a example of the
processing of a simple, non-ambiguous command.
...
\begin{figure}
    ...
    {\sc Dialogs} from the imperative sentence ``Give me the book.''.}
...
interprets common temporal and spatial adverbs (like \emph{above} or
\emph{tomorrow}) and translates simple expressions of internal state (``I'm
tired'') into \emph{experiences} (\stmt{human\_1 experiences state\_1, state\_1
hasFeature tired}, see also Section~\ref{sect:desires}). A full account of the
{\sc Dialogs} features and the corresponding algorithms is available
in~\cite{Lemaignan2011a}.

\subsubsection{Multi-Modality}

Because the components of our architecture rely on the same RDF formalism to
represent their outputs, the different communication modalities (\emph{explicit}
like verbal, deictic or based on gaze, or \emph{implicit} like the position of
the human) are presented in a homogeneous way, as statements in the knowledge
base. The dialogue grounding process makes use of them at two distinct levels to
provide multi-modal concept grounding.

First, specific steps of the grounding process explicitly check for the
...
communication context taking into account the human position).
...
possibly be available in the knowledge base, and the robot fallbacks on the
...
The HATP planning framework extends the traditional HTN (Hierarchical Task
Network) planning domain representation and semantics by making them more
suitable to elaborate plans which involve humans and robots acting together
toward a joint goal. HATP is used by the robot to elaborate human-robot
\emph{shared plans}~\cite{Grosz1996,Clark1996,Kemp2007} which are then used to
anticipate human action, to propose to human to act or even to ask help from
the human.
...
One of the main role of {\sc Shary} is to control the production and execution
...
We finally briefly comment on this design choice and the limitations that we
have observed.
...
Rachid -- TDB: what are the other 'deliberative architecture for socially interactive
robots' and where/how ours is better?

Discussion on deliberative architectures != cognitive architecture: a \emph{cognitive
