...
We identify the needed individual and collaborative cognitive skills: geometric
...
The robot controller \cite{clodic2014key} (with the help of a task planner) decides what action to
...
be sensed and/or produced by the robot to facilitate human-robot joint action
\cite{sebanz2006joint,knoblich20113,pacherie2012phenomenology,vesper2010minimal}.
...
Sections~\ref{sect|discussion} and~\ref{sect|conclusion} finally summarise our
...
\caption{Overview of the architecture. A deliberative layer, composed of six
    ...
    underline the central role of the knowledge base: many of the data streams are
...
full belief state is made explicit (which may be computationally expensive), and
...
controllers {\sc Shary}\cite{clodic2008shary} and {\sc pyRobots}. Note that greater details on each of
...
to recognise simple atomic actions. Those primitive actions are assessed through
...
The hierarchical structure of the shared plans obtained through the HTN refinement process provide a good basis for plan verbalization allowing the robot, when asked, to explain to its human partner how a task can be  shared~\cite{fiore2016planning,milliez2016using}.
...
\caption{An example of plan produced by HATP for a task consisting in
    ...
    The arrow between the two streams represents a synchronization point between 
    the robot and the human based on a causal link.}
...
implemented as filters and that are applied whenever a plan is produced.
In the future we intend to extend the plan-search algorithm and integrate the social rules 
...
achievement~\cite{Rich1997,Sidner2005,sebanz2006joint} in a flexible manner. {\sc Shary} allows
...
Two recent developments have been added that improve robot adaptation to human action at the action and at the plan level.  In~\cite{fiore2014}, the system is able to estimate its human intentions in collaborative tasks and to choose appropriate actions, using a set of POMDP models.  In~\cite{devin2016implemented}, a framework is proposed which allows the robot to estimate its human partner mental state not only about the environment but also about the state of goals, plans and actions and to take them into account when executing human-robot shared plans.
...
\subsubsection{Action Refinement, Execution and Monitoring}\label{sec:action}
...
planning functions provided by a component (called {\sc Mhp},
Figure~\ref{fig|archi}).

These functions are designed to plan navigation~\cite{Sisbot2007} and
manipulation~\cite{Mainprice2011,sisbot2012human} paths not only safe
but also comfortable and “socially acceptable” by reasoning explicitly
on human’s kinematics, vision field, posture and preferences
\cite{Madhava2006,kruse2013human,Rios-MartinezSL15}.  Besides, when necessary, they compute pertinent robot
and objects placements relative to the human\cite{Pandey2011} 

Also, when an action needs motion of the human and the robot, it is
possible to plan for both of them in order to allow the robot to take
initiative by computing automatically where the interaction might
preferably take place\cite{Mainprice2012,waldhart2015planning}, 
thus decreasing the cognitive weight of interaction on human
side. This is somehow the geometric counterpart of HATP.
