...
objects (\ie when a user says ``pick the can'', it resolves to which instance of
\emph{can} the user is referring to) and actions.  Figure~\ref{dialogs|ex} gives
an example of the processing of a simple, non-ambiguous command, while the first
...
are used to classify the sentences into questions, desires or statements, and
{\sc Dialogs} processes them accordingly (by answering questions or
...
\begin{figure}
    ...
    {\sc Dialogs} from the imperative sentence ``Give me the book.'' The new
...
detailed account of our approach to interactive concept clarification and
discrimination, along with the related algorithms.
