...
\begin{keyword}
    human-robot interaction \sep cognitive robotics \sep perspective taking \sep cognitive architecture \sep knowledge representation and reasoning
\end{keyword}
...
\begin{tikzpicture}[
    ...
    \node[anchor=north] at (-6.5,-3.3) {a) Agent-independent\\(allocentric)
        spatial relations};
    \node[anchor=north] at (-2.1,-3.3) {b) Agent-dependent\\(egocentric) spatial
        relations};
    \node[anchor=north] at (1.6,-3.3) {c) Visibility/Pointing};
    \node[anchor=north] at (6.5,-3.3) {d) Reachability};
...
In the current implementation, the social rules are effectively implemented as
filters applied to the set of all possible plans computed by the planner.  In
the future, we intend to extend the plan-search algorithm and integrate the
social rules in the process itself.
