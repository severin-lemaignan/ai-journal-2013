...
\begin{tikzpicture}[
  ...
  \path (lowlevel.east) edge [<-, bend right=80, looseness=1.2] node[label] {elementary\\actions} (shary.east);
...
to recognise simple elemntary actions. Those primitive actions are assessed through
...
mechanisms. In Shary, each action description contains not only the way to execute it and monitor it, but also the way to initiate it (e.g. in the case of a shared action, the way both agents need to commit to the task), to suspend (or cancel or stop) it (and for each case the way this transition has to be shared with the human partner if any). This is supported by a description of the beliefs of each involved agent concerning the action achievement process and a set of monitoring processes allowing to update it
...
They are in fact taken into account in most, if not in all, the
components and at different levels of abstraction: at the shared plan
level as well as at action refinement and execution level.

Repeatedly, the choice of the next action, of who has to act (the human, the robot
or both of them) and when to perform it is made by {\sc Shary} with
the help of {\sc HATP}, based on perspective-taking and
situation assessment ({\sc Spark} and {\sc Oro} as well as on
estimation of the human mental state ({\sc Shary} with the help of
{\sc Oro}).

Where and how to perform an action is made by a full set of on-line
planning functions provided by {\sc Mhp} that computes not only
trajectories but also the pertinent placements and postures based not
only on geometric information but also estimation of the human current
mental state and preferences ({\sc Oro}).
...
of these challenges and making them concrete decisional problems. 
...
This work has been partially supported by EU FP7 "CHRIS" (grant 215805) and ``SAPHARI'' (grant ICT-287513) projects, and the EU H2020 Marie Sklodowska-Curie Actions project DoRoThy (grant 657227).
